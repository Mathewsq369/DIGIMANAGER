...
def generate_ai_image(request, post_id):
    ...
    prompt = request.GET.get("prompt", "A futuristic sunset over the ocean")

    generate_image_gpt4(post, prompt)
    return JsonResponse({"status": "success", "message": "Image generated by GPT-4.1-mini"})

def refine_ai_image(request, post_id):
    post = get_object_or_404(Post, id=post_id)
    prompt = request.GET.get("prompt", "Make it more realistic")
    image_call_id = request.GET.get("image_call_id")  # Pass from frontend or saved ID

    refine_image_gpt4(post, image_call_id, prompt)
    return JsonResponse({"status": "success", "message": "Image refined"})

def generate_sd_image(request, post_id):
    post = get_object_or_404(Post, id=post_id)
    prompt = request.GET.get("prompt", "A dragon curled around a mountain")

    generate_image_sd(post, prompt)
    return JsonResponse({"status": "success", "message": "Image generated by Stable Diffusion"})
